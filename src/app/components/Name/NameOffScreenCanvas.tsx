...
import type { NameCanvasMessage } from "@/workers/nameCanvas.messages";
...
function NameOffScreenCanvas({
  ...
  const canvasRef = useOffscreenCanvasWorker<NameCanvasMessage>({
    name: "nameCanvas",
    init: (canvas) => ({ command: "init", canvas, isMobile }),
    onWorker: (worker, canvas) => {
      // decode the name SVG and hand it over as a texture
      const img = new Image();
      ...
      img.onload = () => {
        ...
        worker.post({
          ...
          image: ctx.getImageData(0, 0, offscreen.width, offscreen.height),
        });
      };

      const handleModeToggle = () => {
        worker.post({ command: "toggleModeUniform" });
      };

      // normalize a client position into [0, 1] over the canvas, flipping y
      ...
      const paintAt = (clientX: number, clientY: number) => {
        ...
        worker.post({ command: "addPoint", point });
        worker.post({ command: "updateMousePosition", mousePosition: point });
      };
