...
function RadioPill({ pillId }: { pillId: string }) {
  return (
    <div>
      <motion.div
        layoutId={pillId + 1}
...
function RadioBrackets({ checked }: { checked: boolean }) {
  const { theme, themeSwitching } = useTheme();
...
function SVGBackground({ activeIndex }: { activeIndex: number }) {
