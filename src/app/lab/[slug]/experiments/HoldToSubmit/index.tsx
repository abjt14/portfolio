...
import { LayoutGroup, cubicBezier, motion } from "motion/react";
import clsx from "clsx";
import { useSoundSettings } from "@/context/SoundProvider";
...
export default function HoldToSubmit() {
  ...
  React.useEffect(() => {
    function handleKeyboardUnfocus(event: KeyboardEvent) {
      if (event.key === "Escape") {
        handleSoftBlur();
      }
    }
...
function ButtonTextAnimation({ trigger }: { trigger: boolean }) {
  return (
    <motion.div
      className="flex px-2"
      layout="position"
...
function ProgressCircleSVG({ isPressed }: { isPressed: boolean }) {
  return (
    <motion.div
      layout={true}
